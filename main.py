...
while True:        
    ...
    if base_light_timer_reset - base_light_timer_start > 600:
        base_light = ldr_reading.ldr_reading()
    ...
    if brush_timer - brushes_completed_reset_timer > 43200: # 12 hour reset timer
        brushes_completed = 0
        brushes_completed_reset_timer = time.time()
    ...
    if light_value>base_light*1.2: # ændret fra 1.3
        ...
        if current_time - start_time > 10 and count < 12:
            start_time = time.time()
            count=count+1
            print(f'count: {count}')
    ...
    if light_value < base_light*1.2 and brush_time_start > 0 or light_value == 0 and brush_time_start > 0:
        ...
        if total_time_brushed > 110 or brushes_completed == 1:
            brushes_completed = 1
            led_ring.led_green()
            
        if total_time_brushed > 110:
            ...
            if object_temp > ambient_temp:
                ...
                if total_time_brushed > 110:
                    print('toothbrush was maybe not held in hand')
        ...
        if total_time_brushed < 110 and brushes_completed == 0:        
            led_ring.led_red()
